...
export class JobsService {
  ...
  async create(createJobDto: CreateJobDto, user: IUser) {
    ...
    } else throw new BadRequestException('Thời gian đặt không hợp lệ!');
  }
  ...
  async findAll(page: number, limit: number, rq: string, req: Request) {
    ...
    const addFilter = await this.authService.handleFetchByHR(req);

    if (addFilter) filter = { ...filter, 'company._id': addFilter };
